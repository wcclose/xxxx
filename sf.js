...
// 文件名：sf_express_auth.js
const $ = new Env('SFExpressAuthorization');
...
async function handleFullURL(fullURL) {
  // 保存完整的URL
  const saveSuccess = await saveDataToEnv('sf_express_url', fullURL);
  if (saveSuccess && !$.isMute) {
    // 发送通知
    $.msg("通知", "顺丰的完整URL", fullURL);
  }
}
